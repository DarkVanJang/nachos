...
{
    bool hasAvailablePid = false; // 能否找到pid
    for(int i = 100; i < MAX_USERPROCESSES; i++)
    {
        if(ThreadMap[i] == false){ 
            ThreadMap[i] = true;
            spaceId = i;
...
// first, set up the translation 
    ...
    for (i = 0; i < numPages; i++) {
	    ...
	    pageTable[i].physicalPage = ProBitmap->Find();  //找到空的页框并分配
