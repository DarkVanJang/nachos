...
Thread::Finish ()
{
    ...
    ASSERT(this == currentThread);
    #ifdef USER_PROGRAM
    waitingThreadExitCode = currentThread->getExitStatus();//获取进程的退出状态
...
Thread::Yield ()
{
    ...
    nextThread = scheduler->FindNextToRun();//找到下一个要运行的线程
    if (nextThread != NULL) {
	scheduler->ReadyToRun(this);//将当前线程放到就绪队列的末尾
...
int Thread::Join(int SpaceId){
    ...
    for(int i = 1; i <= ListLength; i++){
        ...
        else if(thread->UserProgramId == SpaceId){
    ...
    //已经终止
    currentThread->waitProcessExitCode = waitingThreadExitCode;//返回子进程的返回值
    scheduler->deleteTerminatedThread(SpaceId);//删除已终止的子进程
    interrupt->SetLevel(oldLevel);//开中断
}

void Thread::Terminated(){
    List *termainatedList = scheduler->getTerminatedList();//获取终止队列
    Thread *nextThread;
    ASSERT(this==currentThread);
    ASSERT(interrupt->getLevel()==IntOff);
    status = TERNINATED;
    termainatedList->Append((void*)this);
