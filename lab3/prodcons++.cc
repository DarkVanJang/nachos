...
//   page 182 of the textbook.
    ...
    for (num = 0; num < N_MESSG ; num++) {
      ...
      // 制造这个生产品
      message->value = num;
      message->thread_id = which; // which是生产者线程的id
      // Put the code for synchronization before  ring->Put(message) here.
      // ...
      nempty->P(); //同步在前 拿到一个空缓冲区
      mutex->P(); //互斥在后 获得缓冲队列的访问权
      ring->Put(message);
...
Consumer(_int which)
{
    ...
    for (; ; ) {
      ...
      // ...
      nfull->P();  //同步在前 获得一个满缓冲区
      mutex->P();   //互斥在后  获得缓冲队列的访问权限
      ring->Get(message);
...
ProdCons()
{
    ...
    // ....
    mutex = new Semaphore("mutex",1); // 互斥信号量 用于互斥访问
    nempty = new Semaphore("nempty",BUFF_SIZE); // 空缓冲块的数量
    nfull = new Semaphore("nfull",0); // 满缓冲块的数量
    ...
    for (i=0; i < N_PROD; i++) 
    {
      ...
      //  ...
      producers[i]=new Thread(prod_names[i]); // 创建生产者进程，进程名为prod_name
      producers[i] -> Fork(Producer,i); // 启动生产者进程
    };
    ...
    for (i=0; i < N_CONS; i++) 
    {
      ...
      //  ...
      consumers[i] = new Thread(cons_names[i]); // 创建消费者进程
      consumers[i] -> Fork(Consumer,i);  // 启动消费者进程
    };
}
